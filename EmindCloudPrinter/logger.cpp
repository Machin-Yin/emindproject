/*
 * Copyright (C) by Klaas Freitag <freitag@emindprinter.com>
 *
...
void Logger::enterNextLogFile()
{
    if (!_logDirectory.isEmpty()) {
        ...
        QStringList files = dir.entryList(QStringList("emindprinter.log.*"),
                                    QDir::Files);
        QRegExp rx("emindprinter.log.(\\d+)");
        uint maxNumber = 0;
        ...
        QString filename = _logDirectory + "/emindprinter.log." + QString::number(maxNumber+1);
        setLogFile(filename);
    }
}
