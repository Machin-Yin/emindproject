...
class SocketConnect(GObject.GObject):
    ...
    def _update_registered_paths(self, action, args):
        if action == 'REGISTER_PATH':
            ...
            user_dirs = home_path + "/.config/user-dirs.dirs"
            new_line = "XDG_CLOUD_DIR=" + "\"" + args[0] + "\"" + "\n"
            fo = open(user_dirs, "a+")
            for line in fo: 
                if(line.find("XDG_CLOUD_DIR=") >= 0): 
                    old_line = line
                    flag = 1;
            if (flag == 0): 
                fo.write(new_line)
            fo.close()
            if (flag == 1): 
                data = open(user_dirs).read()  
                data = re.sub(old_line, new_line, data)
                open(user_dirs, "wb").write(data)
